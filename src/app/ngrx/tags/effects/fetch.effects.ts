...
export class FetchEffects {
    fetchSubCategoriesByTag$ = createEffect(() =>
        this.actions$.pipe(
            ofType(FetchActions.fetchSubCategoriesByTag),
            switchMap(({ query }) =>
                this.productsService.relatedCategories(query.keyword, query.category).pipe(
                    ...
                    map((res: HttpResponse<IProductSubCategory[]>) =>
                        FetchActions.fetchSubCategoriesByTagSuccess({ subCategories: res.body })
                    ),
                    catchError(err =>
            ...
            switchMap(({ query }) =>
                this.productsService.relatedColors(query.keyword, query.category).pipe(
                    ...
                    map((res: HttpResponse<string[]>) =>
                        FetchActions.fetchColorsByTagSuccess({ colors: res.body })
                    ),
                    catchError(err =>
                        of(FetchActions.fetchFailure({ errorMsg: err.message }))
                    )
        ...
        this.actions$.pipe(
            ...
            switchMap(({ query }) =>
                this.productsService.relatedBrands(query.keyword, query.category).pipe(
                    filter((res: HttpResponse<string[]>) => res.ok),
                    map((res: HttpResponse<string[]>) =>
                        FetchActions.fetchBrandsByTagSuccess({ brands: res.body })
                    ),
                    catchError(err =>
                        of(FetchActions.fetchFailure({ errorMsg: err.message }))
                    )
            ...
            switchMap(({ query }) =>
                this.productsService.relatedPriceRange(query.keyword, query.category).pipe(
                    ...
                    map((res: HttpResponse<number[]>) =>
                        FetchActions.fetchPriceRangeByTagSuccess({ priceRange: res.body })
                    ),
                    catchError(err =>
                        of(FetchActions.fetchFailure({ errorMsg: err.message }))
                    )
