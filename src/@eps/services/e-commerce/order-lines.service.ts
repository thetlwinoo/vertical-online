...
export class OrderLinesService {
  ...
  public extendUrl = SERVER_API_URL + 'services/vscommerce/api/order-lines-extend';

  constructor(protected http: HttpClient) {}

  getOrderLinesByProduct(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IOrderLines[]>(this.extendUrl, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }

  updateExtend(orderLines: IOrderLines): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(orderLines);
    return this.http
      .put<IOrderLines>(this.extendUrl, copy, { observe: 'response' })
